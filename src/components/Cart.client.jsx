...
import {BUTTON_PRIMARY_CLASS} from './Button.client';
export default function Cart() {
  ...
  return (
    <>
      ...
      <div
        className={`z-20 fixed top-0 bottom-0 left-0 right-0 bg-black transition-opacity ${
      ...
      <Dialog open={isCartOpen} onClose={closeCart}>
        ...
        <div
          className={`flex absolute w-96 bg-white rounded flex-col md:block z-20 top-0 left-0 right-0 bottom-0 md:top-7 h-full md:left-auto md:right-7 md:bottom-auto md:h-auto md:max-h-[calc(100vh-56px)] ${
  ...
  return (
    <footer className="bottom-0 sticky pb-8 border-t border-black border-opacity-5">
      <div className="relative h-60 bg-white text-gray-900 p-7">
        <div role="table" aria-label="Cost summery">
...
function LineInCart() {
  const {merchandise} = useCartLine(); //merchandise -> which product byer intrested to.
  return (
    <>
      {/*last: -> last-child selector*/}
      <div
        role="row"
      ...
      >
        ...
        >
          ...
          >
            <CartLineProductTitle className="text-lg font-medium text-red-700" />
          </Link>
...
function CartItemQuantity() {
  return (
    <div className="flex border rounded border-gray-300 items-center overflow-hidden mt-2">
      <CartLineQuantityAdjustButton
        adjust="decrease"
        aria-label="Decrease quantity"
        className="p-2 disabled:cursor-wait"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      <CartLineQuantityAdjustButton
        ...
        className="p-2 text-gray-400 disabled:cursor-wait"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="h-5 w-5"
